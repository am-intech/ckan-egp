...
def _finish(status_int: int,
    ...
    if response_data is not None:
        ...
        if (status_int == 200 and 'callback' in request.args and request.method == 'GET'):
            # escape callback to remove '<', '&', '>' chars
            callback = html.escape(request.args['callback'])
...
def _wrap_jsonp(callback: str, response_msg: str) -> str:
    return '{0}({1});'.format(callback, response_msg)
...
def _get_request_data(try_url_params: bool = False):
    ...
    if not try_url_params and request.method == 'GET':
        raise ValueError('Invalid request. Please use POST method for your request')
    ...
    if not isinstance(request_data, dict):
        raise ValueError('Request data JSON decoded to %r but it needs to be a dictionary.' % request_data)

    if request.method == 'PUT' and not request_data:
        raise ValueError('Invalid request. Please use the POST method for your request')
...
def action(logic_function: str, ver: int = API_DEFAULT_VERSION) -> Response:
    ...
    if not isinstance(request_data, dict):
        ...
        return _finish_bad_request(
            ...
            'Request data JSON decoded to %r but it needs to be a dictionary.' % request_data)
...
def get_api(ver: int = 1) -> Response:
    '''Root endpoint for the API, returns the version number'''
...
def dataset_autocomplete(ver: int = API_REST_DEFAULT_VERSION) -> Response:
    ...
    if q:
        ...
        data_dict: dict[str, Any] = {'q': q, 'limit': limit}

        package_autocomplete = get_action('package_autocomplete')
        package_dicts = package_autocomplete(context, data_dict)

    result_set = {'ResultSet': {'Result': package_dicts}}
...
def tag_autocomplete(ver: int = API_REST_DEFAULT_VERSION) -> Response:
    q = request.args.get('incomplete', '')
    limit = request.args.get('limit', 10)
    vocab = request.args.get('vocabulary_id', '')
    ...
    if q:
        ...
        tag_autocomplete = get_action('tag_autocomplete')
        tag_names = tag_autocomplete(context, data_dict)
    ...
    if q:
        context = cast(
            ...
            {'model': model,
             ...
             'auth_user_obj': current_user})
        data_dict: dict[str, Any] = {'q': q, 'limit': limit}
        format_autocomplete = get_action('format_autocomplete')
        formats = format_autocomplete(context, data_dict)
...
def user_autocomplete(ver: int = API_REST_DEFAULT_VERSION) -> Response:
    ...
    ignore_self = request.args.get('ignore_self', False)
    ...
    if q:
        ...
        data_dict: dict[str, Any] = {'q': q, 'limit': limit, 'ignore_self': ignore_self}

        user_autocomplete = get_action('user_autocomplete')
        user_list = user_autocomplete(context, data_dict)
...
def group_autocomplete(ver: int = API_REST_DEFAULT_VERSION) -> Response:
    ...
    if q:
        ...
        data_dict: dict[str, Any] = {'q': q, 'limit': limit}
        group_autocomplete = get_action('group_autocomplete')
        group_list = group_autocomplete(context, data_dict)
...
def organization_autocomplete(ver: int = API_REST_DEFAULT_VERSION) -> Response:
    ...
    if q:
        context = cast(Context, {
            ...
            'model': model})
        data_dict: dict[str, Any] = {'q': q, 'limit': limit}
        organization_autocomplete = get_action('organization_autocomplete')
        organization_list = organization_autocomplete(context, data_dict)
...
def snippet(snippet_path: str, ver: int = API_REST_DEFAULT_VERSION) -> str:
    ...
    '''
    snippet_path = 'ajax_snippets/' + snippet_path
...
def i18n_js_translations(
    ...
    source = os.path.abspath(os.path.join(ckan_path, 'public', 'base', 'i18n', '{0}.js'.format(lang)))
...
api.add_url_rule('/<int(min=1, max={0}):ver>'.format(API_MAX_VERSION), view_func=get_api, strict_slashes=False)

# Action API (v3)
api.add_url_rule('/action/<logic_function>', methods=['GET', u'POST'], view_func=action)
api.add_url_rule('/<int(min=3, max={0}):ver>/action/<logic_function>'.format(API_MAX_VERSION), methods=['GET', 'POST'], view_func=action)

# Util API
util_rules: list[tuple[str, Callable[..., Union[str, Response]]]] = [
    ('/util/dataset/autocomplete', dataset_autocomplete),
    ('/util/user/autocomplete', user_autocomplete),
    ('/util/tag/autocomplete', tag_autocomplete),
    ('/util/group/autocomplete', group_autocomplete),
    ('/util/organization/autocomplete', organization_autocomplete),
    ('/util/resource/format_autocomplete', format_autocomplete),
    ('/util/snippet/<snippet_path>', snippet),
    ('/i18n/<lang>', i18n_js_translations),
]
